...
public class ShooterSubsystem implements PeriodicRunnable, StateSubsystem<ShooterState> {
    ...
    private static ApplicationProperties applicationProperties = loader.registerPropContainer(ApplicationProperties.class);
    private static ShooterProperties shooterProperties = applicationProperties.getShooterProperties();
...
//        }
    ...
    public enum ShooterState {
        HIGH_SHOOT(shooterProperties.getHighShootPercent()),
        INTAKE(shooterProperties.getIntakePercent()),
        STOP(0d),
        STALL(shooterProperties.getStallPercent()),
        DISABLED(0d);
