...
public class IntakePivotSubsystem implements PeriodicRunnable, PositionalSubsystem {
	...
	//    MAINBOT
	private static final double POT_MIN_VALUE = .4172;
	private static final double POT_MAX_VALUE = .2983;
...
//    private static final double POT_MAX_VALUE = .4503;
	...
	private static final double ANGLE_TO_TAP_LV3 = 129.1333;
...
//    private PidfControlLoop holdingPID = new PidfControlLoop(0.005, 0.008, 0.000, 0, .4);
	private PidfControlLoop holdingPID = new PidfControlLoop(0.02, 0.005, 0, 0, 1);
...
//            zeroFromPot();
        ...
        if(RobotState.isDisabled() && brake){
			intakeMaster.setNeutralMode(NeutralMode.Coast);
			intakeSlave.setNeutralMode(NeutralMode.Coast);
			brake = false;
		}
...
//                + "\t Counter: " + counter.get());
//		System.out.println("COUNTER: " + counter.get() + "\t Position: " + position());
		if (setpoint == null) {
			return;
		}
...
//			intakeMaster.set(ControlMode.PercentOutput, profileManager.getOutput());
	...
	public void setIntakeToClimbHeight(double heightOfHab, double elevatorHeight) {
		...
		set(robotHeightToIntakeAngle(heightOfHab, 21.7 - elevatorHeight + .25));
	}
